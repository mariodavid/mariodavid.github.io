...
If not you are in this weird place where trying to identify the actual problem feels like trying hit a target with a bullet in a totally dark room. Probably the majority of software developers have more or less been in this situation from time to time.
...
It is a very good way to see exactly what entity instances are in your current system so that you can get a better understanding of the problem lies. This is especially true for entities that would otherwise have no user interface.
...
Besides seeing the content, you can also configure the settings of the logging at runtime. If you need to have a higher logging level for one of your classes or are not interested in certain platform logging information, you can just adjust the settings.
...
Actually there are some other features of a CUBA application that will make your life as a application debugger easier like performance statistics or screen profiler. I will not go through all of them, but you might want to scan through the administration menu go get an idea of what can be done.
...
I recently created a application component that took the ideas from the grails console and transferred it to CUBA island.
