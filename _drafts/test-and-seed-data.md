...
You can either select a single or multiple instances and press export. Then, the required zip file, containing the JSON entity definitions will be downloaded. We just have to put it into the right directory, to get it automatically picked up from the JsonDataImporter.
...
## Importing the extracted data
 ...
 So in this case, not only the normal test mode for the cuba application is configured, which allows easier functional testing, but also test data is load so that the functional tests can execute on preconfigured data. Obviously no one prevents us from using this testMode for manual testing as well, so the functional test and manual test will share the test data in this case.
